use {crate::*, std::path::PathBuf, cli_log::*};
...
impl LineConsumer for BaseContent {
    fn start_eating(&mut self, first_date: Date) {
        // self.unfiltered_histogram
        //     .bars
        //     .push(DateBar::new(first_date));
        // self.filtered_histogram.bars.push(DateBar::new(first_date));
    }
    fn eat_line(&mut self, mut log_line: LogLine, _raw_line: &str, filtered_out: bool) {
...
impl LogBase {
    pub fn new(paths: &[PathBuf]) -> Result<Self, NgxLaserError> {
        let mut base_content = BaseContent::default();
        let mut file_reader = FileReader::new(paths, &mut base_content)?;
        ...
        //     return Err(NgxLaserError::NoHitInPaths(paths.to_vec()));
        // }
